...
function Parameters(curve) {
  function getParams(params) {
    let names = Object.keys(params).filter(key => {
    ...
    let values = names.map(name => {
      return formatElement(params[name]);
    });
    return {
      names,
      titles,
  ...
  if (curve.generator !== null) {
    ...
    paramTitles = paramTitles.concat(params.titles, [
      "generator",
      "generator order",
      "cofactor"
    ]);
    paramValues = paramValues.concat(params.values, [
      `(${formatElement(curve.generator.x)}, ${formatElement(
        curve.generator.y
      )})`,
      curve.order,
      curve.cofactor
        ...
        <ul>
          ...
          {anyChars && curve.characteristics.seed !== null && (
            <li>
              <b>Seed</b>:<br />
              <span
                sx={{ variant: "textStyles.mono", overflowWrap: "break-word" }}
              >
                {curve.characteristics.seed}
              </span>
            </li>
          )}
          {anyChars && curve.characteristics.j_invariant !== null && (
            <li>
              ...
              <span
                sx={{ variant: "textStyles.mono", overflowWrap: "break-word" }}
              >
                {curve.characteristics.j_invariant}
              </span>
            </li>
          )}
          {anyChars && curve.characteristics.trace_of_frobenius !== null && (
            <li>
              ...
              <span
                sx={{ variant: "textStyles.mono", overflowWrap: "break-word" }}
              >
                {curve.characteristics.trace_of_frobenius}
              </span>
            </li>
          )}
          {anyChars && curve.characteristics.discriminant !== null && (
            <li>
              ...
              <span
                sx={{ variant: "textStyles.mono", overflowWrap: "break-word" }}
              >
                {curve.characteristics.discriminant}
              </span>
            </li>
          )}
          ...
          {anyChars && curve.characteristics.embedding_degree !== null && (
            <li>
              ...
              <span
                sx={{ variant: "textStyles.mono", overflowWrap: "break-word" }}
              >
                {curve.characteristics.embedding_degree}
              </span>
            </li>
          )}
          ...
          {anyChars && curve.characteristics.conductor !== null && (
            <li>
              ...
              <span
                sx={{ variant: "textStyles.mono", overflowWrap: "break-word" }}
              >
                {curve.characteristics.conductor}
              </span>
            </li>
          )}
        </ul>
...
function Aliases(curve) {
  if (curve.aliases !== null && curve.aliases.length > 0) {
    return (
      <span>
        Also known as:{" "}
        {curve.aliases.map(alias => {
          return (
            <Link to={"/" + alias} sx={{ marginRight: "5px" }}>
              {alias.split("/")[1]}
            </Link>
          );
        })}
...
function SageCode(curve) {
  ...
  if (curve.field.type === "Prime") {
    ...
    if (curve.form === "Weierstrass") {
      ...
      if (curve.generator) {
        sageCode += `G = E(${formatElement(curve.generator.x)}, ${formatElement(
          curve.generator.y
        )})\n`;
      } else {
      ...
      if (curve.generator) {
        sageCode += `G = E(*to_weierstrass(A, B, K(${formatElement(
          curve.generator.x
      ...
      sageCode += `\treturn (x, y)\n`;
      if (curve.generator) {
        sageCode += `G = E(*to_weierstrass(a, d, K(${formatElement(
          curve.generator.x
        )}), K(${formatElement(curve.generator.y)})))\n`;
      } else {
        sageCode += `# No generator defined\n`;
    ...
    if (
      curve.form === "Edwards" ||
      curve.form === "Montgomery" ||
      curve.form === "TwistedEdwards"
    ) {
    ...
    if (curve.form === "Weierstrass") {
      if (curve.field.basis === "poly") {
        ...
        sageCode += `K = GF(2^${
          curve.field.degree
        }, name="x", modulus=${formatPoly(curve.field.poly, true)})\n`;
        sageCode += `E = EllipticCurve(K, (1, K.from_integer(${formatElement(
          curve.params.a
        )}), 0, 0, K.from_integer(${formatElement(curve.params.b)})))\n`;
        sageCode += `E.set_order(${curve.order} * ${curve.cofactor})\n`;
        if (curve.generator) {
          sageCode += `G = E(K.from_integer(${formatElement(
            curve.generator.x
          )}), K.from_integer(${formatElement(curve.generator.y)}))`;
        } else {
          sageCode += `# No generator defined\n`;
        }
        ...
        sageCode += `K.<z> = GF(2^${
          curve.field.degree
        }, name="z", modulus=${formatPoly(curve.field.poly, true)})\n`;
        sageCode += `def from_integer(K, h):\n`;
        sageCode += `\treturn sum(map(lambda ix: K(Integer(ix[1]) * z^(2^ix[0])), enumerate(h.binary())), K(0))\n`;
        sageCode += `E = EllipticCurve(K, (1, from_integer(K, ${formatElement(
          curve.params.a
        )}), 0, 0, from_integer(K, ${formatElement(curve.params.b)})))\n`;
        sageCode += `E.set_order(${curve.order} * ${curve.cofactor})\n`;
        if (curve.generator) {
          sageCode += `G = E(from_integer(K, ${formatElement(
            curve.generator.x
          )}), from_integer(K, ${formatElement(curve.generator.y)}))`;
        } else {
          sageCode += `# No generator defined\n`;
        }
      }
    ...
    if (curve.form === "Weierstrass") {
      ...
      sageCode += `K = GF(${curve.field.base}^${
        curve.field.degree
      }, name="x", modulus=${formatPoly(curve.field.poly, true)})\n`;
      sageCode += `a = K(${formatElement(curve.params.a)})\n`;
      sageCode += `b = K(${formatElement(curve.params.b)})\n`;
    ...
    <div>
      ...
      <div sx={{ display: "flex" }}>
        ...
        <Tooltip title="Download SAGE code" placement="bottom" arrow>
          <div>
            <LinkButton
              href={"data:text/plain," + sageCode}
              download={curve.name + ".sage"}
              sx={{ margin: "20px" }}
            >
    ...
    <div>
      ...
      <div sx={{ display: "flex" }}>
        ...
        <Tooltip title="Download PARI/GP code" placement="bottom" arrow>
          <div>
            <LinkButton
              href={"data:text/plain," + pariCode}
              download={curve.name + ".gp"}
              sx={{ margin: "20px" }}
            >
    ...
    <div>
      ...
      <div sx={{ display: "flex" }}>
        ...
        <Tooltip title="Download JSON" placement="bottom" arrow>
          <div>
            <LinkButton
              href={"data:application/json," + json}
              download={curve.name + ".json"}
              sx={{ margin: "20px" }}
            >
  ...
  return (
    <Entry location={location} title={pageContext.name}>
      ...
      {is_nullundef(data.curve.desc) || data.curve.desc === "" ? null : (
        <div>
          <ReactMarkdown source={data.curve.desc} renderers={{ link: Link }} />
          <br />
        </div>
      )}
      {aliases}
